...
public class DiskBasedCache implements Cache {
    ...
    public synchronized CacheEntry get(String key) {
        ...
        try {
            ...
            RequestLog.d("%s: %s", file.getAbsolutePath(), e.toString());
            remove(key);
            return null;
        } finally {
            if (cis != null) {
    ...
    public synchronized void initialize() {
        if (!mRootDirectory.exists()) {
            if (!mRootDirectory.mkdirs()) {
                RequestLog.e("Unable to create cache dir %s", mRootDirectory.getAbsolutePath());
            }
            return;
        }
    ...
    public synchronized void put(String key, CacheEntry cacheEntry) {
        ...
        try {
            ...
            if (!success) {
                fos.close();
                RequestLog.d("Failed to write header for %s", file.getAbsolutePath());
        ...
        if (!deleted) {
            RequestLog.d("Could not clean up file %s", file.getAbsolutePath());
        }
    }
    ...
    public synchronized void remove(String key) {
        ...
        removeEntry(key);
        if (!deleted) {
            RequestLog.d("Could not delete cache entry for key=%s, filename=%s",
    ...
    private void pruneIfNeeded(int neededSpace) {
        ...
        if (RequestLog.DEBUG) {
            RequestLog.v("Pruning old cache entries.");
        }
        ...
        if (RequestLog.DEBUG) {
            RequestLog.v("pruned %d files, %d bytes, %d ms",
